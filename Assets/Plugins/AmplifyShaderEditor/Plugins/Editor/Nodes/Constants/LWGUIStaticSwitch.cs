...
/* 				if (m_keywordModeType == KeywordModeType.KeywordEnum && m_keywordEnumAmount > 0)
		...
		public string KeywordEnumList(int index)
		{
			...
			{
				if (m_keywordModeType == KeywordModeType.LogicalSubKeywordEnum)
				{
					string strings = m_keywordEnumList[Math.Min(index + 1, m_keywordEnumList.Length - 1)];
		...
		private string GetKeywordEnumPragmaList()
		{
			...
			if (m_keywordModeType == KeywordModeType.LogicalSubKeywordEnum)
			{
				for (int i = 0; i < m_keywordEnumList.Length - 1; i++)
				{
					if (i == 0)
		...
		public override string GenerateShaderForOutput(int outputId, ref MasterNodeDataCollector dataCollector, bool ignoreLocalvar)
		{
			...
			if (/* node.KeywordModeTypeValue == KeywordModeType.KeywordEnum ||  */m_keywordModeType == KeywordModeType.EnumGroup || m_keywordModeType == KeywordModeType.LogicalSubKeywordEnum)
			{
				...
				if(m_keywordModeType == KeywordModeType.LogicalSubKeywordEnum)
				{
					...
					for (int i = 1; i < node.KeywordEnumAmount; i++)
						allOutputs[i] = m_inputPorts[i].GeneratePortInstructions(ref dataCollector);

					for (int i = 1; i < node.KeywordEnumAmount; i++)
					{
						string keyword = node.KeywordEnum(i-1);
						if (i == 1)
		...
		public string CurrentKeyword
		{
			get
			{
                ...
                string[] newstringArray = m_currentKeyword.Replace(" ","").Split(",");
				string newstring = newstringArray[newstringArray.Length - 1];
				string newstring01 = (m_lockKeyword || string.IsNullOrEmpty(m_currentKeyword) ? PropertyName + OnOffStr : newstring);
				if(newstring01.ToCharArray().Any(char.IsUpper))
					return newstring01;
				return " ";
			}
		}
