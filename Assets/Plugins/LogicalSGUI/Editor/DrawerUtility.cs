...
{
	/// when LogicalSGUIEventType.Init:		get all metadata from drawer
	/// when LogicalSGUIEventType.Repaint:	LogicalSGUI decides how to draw each prop according to metadata
	internal enum LogicalSGUIEventType
	{
		Init,
		Repaint
	}
	...
	internal class LogicalSGUI : ShaderGUI
	{
		...
		public LogicalSGUIEventType LogicalSGUIEventType = LogicalSGUIEventType.Init;
		public Shader shader;

		public Texture2D XX;

		/// <summary>
		/// Called when switch to a new Material Window, each window has a LogicalSGUI instance
		/// </summary>
		public LogicalSGUI() { }

		public override void OnGUI(MaterialEditor materialEditor, MaterialProperty[] props)
		{
			// Rect rect00 = EditorGUILayout.GetControlRect(true, 10, EditorStyles.layerMaskField);
			// EditorGUI.EnumPopup(rect00,new UnityEngine.Rendering.CullMode());
			// UnityEngine.Rendering.ColorWriteMask
			this.props = props;
			this.materialEditor = materialEditor;
			this.shader = (materialEditor.target as Material).shader;
			this.LogicalSGUIEventType = RevertableHelper.InitAndHasShaderModified(shader, materialEditor.target, props) ? LogicalSGUIEventType.Init : LogicalSGUIEventType.Repaint;

			// reset caches and metadata
			if (LogicalSGUIEventType == LogicalSGUIEventType.Init)
			{
	...
	internal class Helper
	{
		...
		/// Unity 2019.2+
		/// </summary>
		public static LogicalSGUI GetLogicalSGUI(MaterialEditor editor)
		{
			if (editor.customShaderGUI != null && editor.customShaderGUI is LogicalSGUI)
			{
				LogicalSGUI gui = editor.customShaderGUI as LogicalSGUI;
				return gui;
			}
			else
			{
				Debug.LogWarning("Please add \"CustomEditor \"LogicalSGUI.LogicalSGUI\"\" to the end of your shader!");
				return null;
			}
		}
		...
		public static Rect[] SplitRect(Rect rectToSplit, int n)
		{
			...
			int padding = (int)rects[0].width - 48; // use 50, enough to show 0.xx (2 digits)
			int space = 0;
			rects[0].x = rects[0].x - 15;
			rects[0].width -= padding + space;
			rects[2].width -= padding + space;

			rects[1].x = rects[1].x - padding - 28;
			rects[1].width += padding * 2 + 40;
		...
		public static bool EnumFoldout(Rect position, ref bool isFolding, bool toggleValue, bool hasToggle, GUIContent label)
		{
			...
			var toggleRect = new Rect(rect.x - 110 + position.width, rect.y + 7f, 13f, 13f);
			...
			if (hasToggle)
			{
				...
				GUI.Label(new Rect(rect.x - 162 + position.width, rect.y + 7f, 60f, 13f), "高级参数");
			...
			if (EditorGUIUtility.currentViewWidth >= logoRect.width)
			{
				...
				if (logoRect.Contains(Event.current.mousePosition))
				{
					...
					if (Event.current.type == UnityEngine.EventType.MouseDown)
						Application.OpenURL("https://github.com/JasonMa0012/LogicalSGUI");
				}
				GUI.DrawTexture(logoRect, _logo);
				GUI.color = c;
				GUI.Label(logoRect, new GUIContent(String.Empty, "LogicalSGUI (Light Weight Shader GUI)\n\n"
		...
		public static bool DrawSearchField(ref string searchingText, ref SearchMode searchMode, LogicalSGUI logicalSgui)
		{
#if UNITY_2023_1_OR_NEWER
			var toolbarSeachTextFieldPopup = new GUIStyle();
#else
			...
			if (Event.current.type == UnityEngine.EventType.MouseDown && modeRect.Contains(Event.current.mousePosition))
			{
				EditorUtility.DisplayCustomMenu(rect, _searchModeMenus, (int)searchMode,
												...
												{
													if (logicalSgui.searchMode != (SearchMode)selected)
													{
														logicalSgui.searchMode = (SearchMode)selected;
														logicalSgui.updateSearchMode = true;
													}
												}, null);
				Event.current.Use();
			}
	...
	internal class RampHelper
	{
		...
		{
			get { return EditorPrefs.GetString("LogicalSGUI_GradientSavePath_" + Application.version, Application.dataPath); }
			set
			{
				if (value.Contains(projectPath))
					EditorPrefs.SetString("LogicalSGUI_GradientSavePath_" + Application.version, value);
			}
		}
		...
		public static Gradient GetGradientFromTexture(Texture texture, out bool isDirty, bool doReimporte = false)
		{
			...
			if (assetImporter != null && assetImporter.userData.Contains("LogicalSGUI"))
			{
	...
	internal static class MaterialUtils
	{
		...
		public static void SetupBlendMode(Material material, float index)
		{
			switch (index)
			{
				case 0f:
					// material.SetOverrideTag("RenderType", "Opaque");
					// material.shader.maximumLOD = 500;
					// material.DisableKeyword("_ALPHATEST_ON");
					// material.renderQueue = 2000;
					// Debug.Log(material.GetShaderPassEnabled("ClipPreDepth"));
					// Debug.Log(material.ToString());
					break;
				case 1f:
					// material.SetOverrideTag("RenderType", "TransparentCutout");
					// material.SetShaderPassEnabled("ClipPreDepth",false);
				...
				case 2f:
					...
					// material.SetShaderPassEnabled("ClipPreDepth",true);
					// Shader newShader = material.shader;
					// newShader.maximumLOD = 400;
