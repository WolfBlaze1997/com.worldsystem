...
SpeedTreeVertexOutput vert(SpeedTreeVertexInput input)
{
...
#ifdef GBUFFER
    FragmentOutput frag(SpeedTreeFragmentInput input)
#else
    half4 frag(SpeedTreeFragmentInput input) : SV_Target
#endif
{
    ...
    half4 extra = tex2D(_MRAMap, uv);
    metallic = saturate(extra.r * _Metallic);
    smoothness = saturate((1.0 - extra.g) * _Smoothness);
    ...
    SETUP_DEBUG_TEXTURE_DATA(inputData, input.interpolated.uv, _BaseMap);
    ...
    // subsurface (hijack emissive)
        ...
        float3 directSubsurface = tintedSubsurface.rgb * mainLight.color.rgb * fSubsurface * realtimeShadow * tex2D(_SubsurfaceTex, uv).a ;
        float3 indirectSubsurface = tintedSubsurface.rgb * inputData.bakedGI.rgb * _SubsurfaceIndirect * tex2D(_SubsurfaceTex, uv).a ;
