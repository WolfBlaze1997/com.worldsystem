...
TEXTURE2D_X_FLOAT(_CameraDepthTextureAddCloudMask);
SAMPLER(sampler_CameraDepthTextureAddCloudMask);
...
float GetLinearDepthValue(float2 uv)
{
    ...
    float depth = LOAD_TEXTURE2D_X_LOD(_CameraDepthTextureAddCloudMask, uint2(uv * _ScreenSize.xy), 0).x;
#else
    float depth = LOAD_TEXTURE2D_X_LOD(_CameraDepthTextureAddCloudMask, uint2(uv * GetScaledScreenParams().xy), 0).x;
#endif
