...
**求职技术美术或相关岗位,两年技术美术工作经验,三年次世代建模工作经验.

现在真的很需要一份工作,希望能长期稳定,工资都好商量

我现在在重庆,工作地点最好在成都或重庆,其他地方也可以接受

如果给我推荐公司,转正后我愿意给半个月工资做为感谢
...
# **已更新到1.0,比较完整了,大量更新,文档还没写的**

![](./Documentation~/mianban.png)

# 简介
- 简单易用的高性能昼夜天气系统 (体积云方案), 采用原创的异步+分帧渲染技术, 旨在提供极致的游戏性能. 
- 完全模块化的架构, 便于开发者轻松扩展和定制功能, 以满足不同游戏开发的需求 , 这也是包名 "WorldSystem" 包含的 "野心", 我希望它不仅仅是一套昼夜天气的方案, 而是包含游戏需要的各种渲染效果, 它们都会以模块提供, 这是一个长期目标.
- 实现了专为开发者和使用者设计的不同操作面板, 通过切换开发者模式, 优化了调试体验, 同时保证了非开发环境下的界面整洁与安全性.
- 创建了包管理器, 集中管理个人开发的各类包,工具和库, 提升了资源的整合与使用效率.
